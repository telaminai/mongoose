...
6. **Durable consumer-side audit replay** — **generalise the existing `MongooseAuditCaptureService` / `ChronicleAuditCaptureService` to a second record type (`ReplayRecord`)** rather than building new; add a redirectable replay `Auditor` (build-time) whose sink `start()` flips to Chronicle (dodges the single-slot swap). `durableAuditReplay` per-processor; detached-copy serialisation of `PoolAware`; snapshot integration by *reference* to Fluxtion state-recovery; extend the svc-admin-web Replay tab + introspection split. *(The determinism/recovery artifact — independent of the bus.)*
