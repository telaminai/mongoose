...
**Status**: Draft / problem-framing — rev 5 (four review rounds; consumer-side-replay reframe; spec-precision + consistency sweep). Reviewers converge on "ready to split into implementation specs and start phases 1–3." No code yet.
...
6. **Durable consumer-side audit replay** — Chronicle-backed `ReplayRecordWriter` / `AuditReplayStore`; `durableAuditReplay` per-processor; detached-copy serialisation of `PoolAware`; snapshot integration by *reference* to Fluxtion state-recovery. *(The determinism/recovery artifact — independent of the bus.)*
...
- *(new)* **Chronicle-backed `ReplayRecordWriter` / `AuditReplayStore`** — a durable `Auditor` sink for the **consumer-side** audit stream (the determinism/recovery artifact); `processor.durableAuditReplay` / `auditReplayRetention`; detached-copy serialisation of `PoolAware` events. Refs: `fluxtion/.../replay/YamlReplayRecordWriter.java`, `YamlReplayRunner.java`, `docs/how-to/replay-functionality.md`, `docs/.../state-and-recovery.md`.
