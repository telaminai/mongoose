...
| `MongooseServerConfig` (+ `Builder`) | `builder`, `addProcessor(String,T,String)`; builder `addEventFeed/addEventSink/build` |
...
| `FileEventSource` | no-arg ctor, `setFilename` (plugin `connector.file` package) |
...
that will not build locally. A representative `MongooseMain` already
